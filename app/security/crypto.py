...
The master key comes from `FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY` (base64 or hex
encoded 32 bytes).  Additional keys for rotation can be supplied as
`FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY_V<n>`; the highest version is used for new
...
def _decode_key(raw: str) -> bytes:
    raw = raw.strip()
    if not raw:
        raise CredentialCryptoError("FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY is not set")
...
class CredentialCipher:
    ...
    def from_env(cls, env: Optional[Dict[str, str]] = None) -> "CredentialCipher":
        ...
        keys: Dict[int, bytes] = {}
        base = env.get("FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY", "")
        if base:
            keys[1] = _decode_key(base)
        for name, value in env.items():
            if name.startswith("FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY_V") and value:
                try:
        ...
        if not keys:
            ...
            if settings.is_dev and not settings.FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY:
                # Deterministic dev key so local runs work without setup; never used in prod.
                keys[1] = b"\x01" * 32
            else:
                raise CredentialCryptoError(
                    "FLOWBOARD_CREDENTIAL_ENCRYPTION_KEY is required (generate one with "
